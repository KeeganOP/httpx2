...
def test_base_url_with_params_and_request_params():
    # Query params in the base URL must not be dropped
    client = httpx2.Client(base_url="https://api.example.com/v1?first=base_url")
    request = client.build_request("GET", "users", params={"page": "2"})

    assert str(request.url) == "https://api.example.com/v1/users?first=base_url&page=2"
